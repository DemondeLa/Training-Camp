...
【【史上最全】MySQL各种锁详解：1小时彻底搞懂MySQL的各种锁】https://www.bilibili.com/video/BV1po4y1M7k5?vd_source=a4fb55235c3c85bbc703dee027f5ec09
...
锁是计算机在执行多进程或线程时用于并发访问同一共享资源时的同步机制，MySQL中的锁是在服务层或者存储引擎层实现的，保证了数据访问的一致性与有效性。
...
MySQL全局锁是针对整个数据库的锁。在数据库中加锁的目的是为了保证在一段时间内数据的一致性，不会出现数据错乱的问题。最常用的全局锁是读锁和写锁。

1. 读锁（共享锁）：它阻止其他用户更新数据，但允许他们读取数据。这在你需要在一段时间内保持数据一致性时很有用
2. 写锁（排它锁）：它阻止其他用户读取和更新数据。这在你需要修改一些大量的数据，并且不希望其他用户在这段时间内干扰时很有用

MySQL全局锁的典型使用场景是，进行一些需要确保整个数据库一致性的操作，例如全库备份、全库导出等。

![](pic/lock_1.png)

> 如上图，假设数据库中有2张表，一张是用户余额表、一张套餐表，（表中的数据如左边所示），如果此时开启了全局备份（假设没有开启全局锁），（一张表一张表备份），首先备份余额表，在备份的同时购买了套餐（98超值双人套餐），此时就出现了问题，钱没有扣，而套餐已经发生了改变。
>
> 这就是全局锁的目的。

在MySQL中，可以使用`FLUSH TABLES WITH READ LOCK(FTWRL)`语句来添加全局读锁，这将阻止其他线程进行更新操作。使用`UNLOCK TABLE`语句来释放锁定

请注意，全局锁的开销非常大， 因为它会阻止其他所有的数据修改操作，并且在高并发情况下可能导致大量的线程等待锁定。因此，应该尽量避免在生产环境中使用全局锁，或者尽量减少全局锁的持有时间

虽然全局锁有其应用场景，但是过度使用或不正确使用全局锁可能导致性能问题。因此，根据应用的特性和需求选择适合的锁策略是很重要的。对于大多数应用，优先使用更精细粒度的锁，如行锁和表锁，可以更有效地处理并发请求，同时避免全局锁的开销。

在<u>**InnoDB引擎**</u>中，有多版本并发控制MVCC机制，通过这个机制也可以保证数据的一致性。在整库备份导出的时候，也可以使用如下命令：在保证数据一致性的情况下，不加全局锁进行数据的导出

```sql
mysqldump --single-transaction -uroot -p example(数据库名) > example.sql(导出的文件名)
```
...
在MySQL中，对MyISAM表的读操作，会自动加上读锁，对MyISAM表的写操作，会自动加上写锁。因为在MyISAM中是没有事务的，那么在面对竞争激烈的情况向下，没有办法保证数据的一致性，只能粗暴的使用这种方式来保证
...
4. **全表更新或者删除**：这一种可能是在InnoDB中使用的最多的一种。在某些情况下，可能需要对一张表进行全表的更新或者删除操作，例如，删除表中的所有记录，或者更新表中所有记录的某个字段的值，这些操作是一定会将该表上锁的，来保证该表不会被其他人修修改。在这种情况下，使用表级锁是合适的。
...
3. `LOCK TABLES XXX(表名) read/write`：这个命令可以显式地为一个或多个表加上读锁或写锁。LOCK TABLES命令后面可以跟上一系列的表名和锁模式，用来指定需要锁定哪些表，以及使用什么样的锁模式。例如，LOCK TABLES t1 WRITE, t2 READ;命令会给表t1加上写锁，给表t2加上读锁，使用`unlock tables`进行解锁。
...
1. 死锁：当两个或更多的事务相互等待对方释放资源时，就会发生死锁，所以粒度越小，也就更容易发生死锁，即控制的资源越多。例如，事务1锁定了行A并试图锁定行B，同时事务2锁定了行B并试图锁定行A，这就形成了死锁。MySQL会检测到死锁并终止其中一个事务，但这仍可能导致性能问题和事务失败。
...
4. 资源消耗：行级锁（粒度越小）需要更多的内存来存储锁信息，而且需要更多的CPU时间来处理锁请求和释放锁。如果数据库中的行数非常多，或者并发事务的数量非常多，这可能会导致显著的资源消耗。
...
乐观锁和悲观锁，并不像上面的粒度锁一样能够在MySQL中具体的体现出来，它们是两种锁的思想

#### 乐观锁

乐观锁（Optimistic Locking）是一种在数据库操作中用于处理并发问题的技术。

它的基本思想是假设在多个事务同时访问同一条数据时，冲突发生的概率较低，因此在操作数据时不会立即进行锁定（体现了乐观的思想），而是在提交数据更改时检查是否有其他事务修改了这条数据。如果没有，就提交更改，否则就回滚事务。
...
假设我们有一个名为Products的表，其中有一个version字段用于实现乐观锁：
...
3. 更新数据：更新数据时，需要检查版本号是否仍然和原来读取的版本号一致。如果一致，就增加版本号并更新数据；如果不一致，说明数据已经被其他事务修改过，因此应该回滚当前事务。 
    ...
    其中，:oldVersion是参数，应该替换为原来读取的版本号。 
...
在MySQL中，共享锁（Shared Lock）是通过SQL语句实现的。具体来说，你可以使用SELECT ... LOCK IN SHARE MODE语句来对选定的行加共享锁。
例如：
...
需要注意的是，在MySQL中，所有的锁都是在事务中使用的，并且在事务结束时会自动释放。如果你在非事务环境中使用锁，那么锁会在语句执行完毕后立即释放，这可能会导致数据的不一致。
...
在MySQL中，使用共享锁（Shared Lock）的主要方式是使用SELECT ... LOCK IN SHARE MODE语句。下面是一个使用共享锁的基本示例：
...
1. LOCK IN SHARE MODE应在事务中使用。在事务结束（提交或回滚）时，MySQL会自动释放所有锁。
2. 使用LOCK IN SHARE MODE的时候，你需要保证你的存储引擎支持行级锁，比如InnoDB。MyISAM这种表级锁的存储引擎是不支持的。
3. 如果你的事务隔离级别是READ UNCOMMITTED，那么LOCK IN SHARE MODE可能不会按照你期望的方式工作，因为在这个隔离级别下，事务可以读取其他事务未提交的修改。在READ COMMITTED，REPEATABLE READ，和SERIALIZABLE隔离级别下，LOCK IN SHARE MODE能正常工作。
...
- 意向锁的设计旨在提供一种有效的锁机制来优化锁的管理。它允许数据库系统在不同粒度的锁之间有效地协调，从而避免了对整个表或整个数据集加锁时带来的性能瓶颈。
- 意向锁减少了锁检查的开销。因为如果一个表上有意向共享锁或意向排他锁，数据库系统可以在进行更细粒度的锁检查（如行级锁）时快速判断是否存在锁冲突，而不需要检查每一行的具体状态。
...
意向共享锁表示一个事务打算在某些行上申请共享锁。共享锁允许事务读取数据但不修改数据。如果一个事务对某一行加了共享锁，其他事务可以在该行加共享锁，但不能加排他锁。

**用途：**

- 意向共享锁主要用于告知系统，当前事务希望在某些行上加共享锁。这样，数据库在检查锁的兼容性时可以知道这个事务的意图。
- 它的作用是让数据库系统知道，事务可能在更细粒度的锁上（例如行锁）请求共享锁。

**例子：** 假设事务T1在表上加了意向共享锁（IS）。这表示T1可能会在表的某些行上请求共享锁。其他事务可以在表级上加意向排他锁（IX），但不能加排他锁。

##### 意向排他锁

意向排他锁表示一个事务打算在某些行上加排他锁。排他锁不允许其他事务同时对这些行加共享锁或排他锁。通过设置意向排他锁，事务通知数据库系统它将对某些行进行排他操作。

**用途：**

- 意向排他锁用于告知系统，当前事务打算在某些行上加排他锁，从而避免其他事务在同一层级上加排他锁。
- 它是一种较高层次的锁，通常用于表级锁。当一个事务想要对表中某些行加排他锁时，它会先在表级加意向排他锁。

**例子：** 假设事务T1在表级上加了意向排他锁（IX），这表示它可能会在表中的某些行上加排他锁。其他事务可以在表级加意向共享锁（IS），但不能加排他锁。

##### 兼容互斥性

| 锁类型               | 意向共享锁（IS） | 意向排他锁（IX） | 共享锁（S） | 排他锁（X） |
| -------------------- | ---------------- | ---------------- | ----------- | ----------- |
| **意向共享锁（IS）** | 兼容             | 不兼容           | 兼容        | 不兼容      |
| **意向排他锁（IX）** | 不兼容           | 兼容             | 不兼容      | 不兼容      |
| **共享锁（S）**      | 兼容             | 不兼容           | 兼容        | 不兼容      |
| **排他锁（X）**      | 不兼容           | 不兼容           | 不兼容      | 不兼容      |

- **意向共享锁**与其他事务的**共享锁**兼容，但与排他锁不兼容。
- **意向排他锁**与其他事务的**排他锁**和**共享锁**都不兼容，避免了其他事务对同一资源加锁

##### 意向锁的应用场景

意向锁广泛应用于数据库的行级锁和表级锁之间的协调。它在事务并发执行时，通过提高锁的粒度灵活性，确保锁管理更高效，并减少了锁冲突，提升了数据库的并发性和性能。

总结：

- **意向共享锁（IS）**：表示一个事务希望在某些行上加共享锁（读锁），它是对行级锁的一种声明。
- **意向排他锁（IX）**：表示一个事务希望在某些行上加排他锁（写锁），它是对行级锁的一种声明。

这两种锁通常与行级锁和表级锁的配合使用，能够有效减少锁的冲突，提高并发性能。
...
Next-Key 可以理解为一种特殊的间隙锁，也可以理解为一种特殊的算法。通过临建锁可以解决幻读的问题。 每个数据行上的非唯一索引列上都会存在一把临键锁，当某个事务持有该数据行的临键锁时，会锁住一段==左开右闭区间==的数据。需要强调的一点是，InnoDB 中行级锁是基于索引实现的，临键锁只与非唯一索引列有关，在唯一索引列（包括主键列）上不存在临键锁。
